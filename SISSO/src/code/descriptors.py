...
def process_descriptor(folder_list, desc, train, test, label, seed):
    ...
    fold_accuracy, fold_accuracy_o, fold_accuracy_x = [], [], []
    fold_f1 = []
    ...
    for num, folder in enumerate(folder_list):
        ...
        fold_f1.append(f1_score(fold_test_y, y_pred, average='weighted'))
        fold_accuracy.append(accuracy_score(fold_test_y, y_pred))
        fold_accuracy_o.append(accuracy_score(fold_test_yo, y_pred_o))
        fold_accuracy_x.append(accuracy_score(fold_test_yx, y_pred_x))
    ...
    return accuracy_test, f1_test, accuracy_train, f1_train, np.mean(np.array(fold_accuracy)), np.mean(np.array(fold_f1)), np.mean(np.array(fold_accuracy_o)), np.mean(np.array(fold_accuracy_x))
...
class Out:
    def __init__(self,
        ...
        # descriptor들의 정확도 계산
        self.get_accuracy()
        self.descriptor_df = self.descriptor_df.sort_values(by='Accuracy_avg_fold_test', ascending=False).reset_index(drop=True)
        self.descriptor_df = self.descriptor_df.sort_values(by='Accuracy_test', ascending=False).reset_index(drop=True)
    ...
    def all_descriptor(self, fold_path):
        ...
        with open(features_space, 'r') as file:
            self.descriptors += [line.split('N=')[0].strip() for line in file]
